...
format_payload(Message) ->
    io:format("format message to kafka"),
    Username = emqx_message:get_header(username, Message),
    io:format("format Username ~s~n",[Username]),
    
    Deviceid = Message#message.from,
    io:format("format Deviceid ~s~n",[Deviceid]),
    
    %Ts = Message#message.timestamp,
    
    %io:format("format Ts ~s~n",[Ts]),
    ...
    % io:format("MsgPayload64 : ~s~n", [MsgPayloads]);
        RawType == false ->
            MsgPayloads = MsgPayload
    end,
...
produce_kafka_payload(Message) ->
    ...
    ok = ekaf:produce_async_batched(Topic, Payload),
